...
<script type="text/javascript">
	...
	$(function() {
       ...
       $(".area-one").click(function(){
           window.location.href = "orderlist.php?customer_id="+customer_id_en+"&currtype="+$(this).data("type")+"&user_id="+user_id_en;
       });
	});
    ...
    function searchData() {
        ...
        $.ajax({
            ...
            data: "pagenum="+(pageNum+1)+"&currtype="+ctype+"&user_id="+user_id_en,
    ...
    function order_pay(batchcode,totalprice){
        ...
        if(iptCurrency != "" && open_curr == 1){
            if(parseFloat(iptCurrency) > parseFloat(totalprice)){
                showAlertMsg("操作提示","最多只能使用"+totalprice+"个购物币","知道了");
                return;
            }
            if(parseFloat(iptCurrency) == parseFloat(totalprice)){ //全部使用购物币支付
                showConfirmMsg("操作提示","是否确定全部使用购物币支付？","支付","取消",function(){
                    $.ajax({
                        type: "get",
