...
if(!empty($_GET["type_imgurl"])){
    $type_imgurl=$configutil->splash_new($_GET["type_imgurl"]);
	 ...
	 if($customer_id>0){
	   $query2="select index_imgurl from weixin_commonshop_types where isvalid=true and id=".$keyid." and customer_id=".$customer_id;
	   //echo $query2;
		$result2 = mysql_query($query2) or die('Query failed2: ' . mysql_error());
		while ($row2 = mysql_fetch_object($result2)) {
		   $type_imgurl=$row2->index_imgurl;   
		}
	}
}
$op="";
if(!empty($_GET["op"])){
   ...
   if($op=="del"){  
   $keyid = $configutil->splash_new($_GET["keyid"]);
		  $query="update weixin_commonshop_types set index_imgurl='' where id=".$keyid;	
//echo $query;  
		  mysql_query($query);
...
<span class="upload_file">
	<div>
	   <form action="save_pro_logo.php?customer_id=<?php echo $customer_id_en; ?>" id="frm_img" enctype="multipart/form-data" method="post">
			<div class="up_input">
			<input name="upfile" id="upfile" type="file"  width="120" height="30" value="Submit" onchange="uploading()">
			<div id="xianshi" style="display:none;margin:50px 0px 0px 135px;"><img src="images/upload.gif"></div>
...
<script type="text/javascript">  
  function uploading(){   
	...
	function delImg(id){
	   url = "product_type_catimg.php?op=del&keyid="+id+"&customer_id=<?php echo $customer_id_en; ?>";
